...
dj <- read.csv("Map/대전자치단체.csv", fileEncoding='UTF-8')

kakao_api_key <- readLines('OpenAPI/kakao_api_key.txt', encoding='UTF-8')

base_url <- "https://dapi.kakao.com/v2/local/search/address.json"
keyword <- URLencode(iconv('전북 삼성동 100', to='UTF-8'))
...
leaflet(dj) %>% 
  setView(lng=mean(dj$lng), lat = mean(dj$lat), zoom = 12) %>% 
  addTiles() %>% 
  addCircles(lng=~lng, lat=~lat,label=~place,popup=~addr,weight=1,
             radius=~pop/1000, color=~color)
